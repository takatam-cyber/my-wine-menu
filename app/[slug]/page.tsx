...
export default function StoreMenu() {
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] pb-32 text-slate-900">
      ...
      <div className="max-w-2xl mx-auto px-5 pt-8 space-y-8">
        {filteredWines.map(wine => (
          <div key={wine.id} onClick={() => setSelectedWine(wine)} className={`relative bg-white rounded-[2rem] overflow-hidden shadow-xl border-2 transition-all active:scale-95 cursor-pointer ${wine.is_priority ? 'border-amber-400' : 'border-transparent'}`}>
            {wine.is_priority && <div className="absolute top-4 left-4 z-10 bg-amber-400 text-black text-[10px] font-black px-3 py-1 rounded-full shadow-lg">★ RECOMMENDED</div>}
      ...
      {selectedWine && (
        <div className="fixed inset-0 z-[70] bg-black/40 backdrop-blur-md flex items-end animate-in fade-in">
          <div className="bg-white w-full max-h-[90vh] rounded-t-[3rem] overflow-y-auto pb-12 shadow-2xl relative">
            ...
            <div className="p-8 space-y-6">
              <img src={selectedWine.image_url} className="w-full aspect-square object-contain bg-slate-50 rounded-3xl shadow-inner" />
              ...
              <button onClick={() => {setChatOpen(true); setSelectedWine(null);}} className="w-full py-4 bg-slate-900 text-white rounded-xl font-bold">このワインをAIに詳しく聞く</button>
            </div>
          </div>
        </div>
      )}
      ...
      {chatOpen && (
        <div className="fixed inset-0 bg-white z-[80] flex flex-col animate-in slide-in-from-bottom duration-300">
          <header className="p-6 border-b flex justify-between items-center bg-white sticky top-0 shadow-sm">
            ...
            <button onClick={() => setChatOpen(false)} className="p-2 bg-slate-100 rounded-full"><X size={24}/></button>
          </header>
                      ...
                      {Array.from(h.content.matchAll(/【ID:(\d+)】/g)).map(m => {
                        ...
                        return w && (
                          <button key={w.id} onClick={() => { setSelectedWine(w); setChatOpen(false); }} className={`w-full p-3 rounded-xl border-2 flex gap-3 text-left ${w.is_priority ? 'border-amber-400 bg-amber-50' : 'bg-slate-50'}`}>
                            <img src={w.image_url} className="w-12 h-16 object-cover rounded shadow-sm" />
                            <div className="flex-1 min-w-0 flex flex-col justify-center"><p className="text-xs font-black truncate">{w.name_jp}</p><p className="text-sm font-black text-slate-500">¥{Number(w.price_bottle).toLocaleString()}</p></div>
          ...
          <div className="p-6 border-t bg-white flex gap-2">
            ...
            <button onClick={handleChat} disabled={!message.trim()} className="bg-slate-900 text-white p-4 rounded-xl shadow-lg active:scale-95 disabled:opacity-50"><Send/></button>
          </div>
        </div>
      )}
    </main>
