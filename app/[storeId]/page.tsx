    ...
    <main className="min-h-screen bg-[#0d0e12] text-[#c5a059] font-serif pb-24">
      ...
      <div className="max-w-xl mx-auto px-6 space-y-24 mt-16">
        {wines.filter(w => parseInt(w.stock) > 0).map((wine: any) => (
          <div key={wine.id} className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
        ...
        {wines.length === 0 && <p className="text-center text-slate-600 font-sans font-bold tracking-[0.4em] py-48 opacity-50 uppercase text-xs">Cellar is currently empty</p>}
      </div>
    </main>
  );
}
