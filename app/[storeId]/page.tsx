export const runtime = 'edge'; // Cloudflare Pagesで動かすために必須の一行
...
export default function StoreMenu() {
  ...
  return (
    <main className="min-h-screen bg-[#0d0e12] text-[#c5a059] font-serif pb-24">
      <header className="py-20 px-6 text-center border-b border-[#c5a059]/10">
      ...
      <div className="max-w-xl mx-auto px-6 space-y-24 mt-16">
        {wines.filter(w => parseInt(w.stock) > 0).map((wine: any) => (
          <div key={wine.id} className="space-y-10 group animate-in fade-in slide-in-from-bottom-4 duration-1000">
            {/* 高級感のあるメインカード */}
            <div className="aspect-[3/4] rounded-sm overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.8)] relative border border-white/5 bg-[#1a1c23]">
              {wine.image ? (
                <img src={wine.image} alt={wine.name_jp} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" />
              ) : (
              ...
              <div className="absolute bottom-10 left-10 right-10 flex justify-between items-end">
                <div className="space-y-1 pr-4">
                  <p className="text-[10px] tracking-[0.4em] font-sans font-black uppercase text-[#c5a059] opacity-80">
                    {wine.country} / {wine.vintage}
                  </p>
                  <h2 className="text-3xl text-white font-light leading-tight">{wine.name_jp}</h2>
                </div>
                <div className="text-right">
                  <p className="text-2xl font-sans text-white font-bold tracking-tighter">
            ...
            <div className="space-y-8 px-2">
              ...
              <div className="relative p-10 bg-white/2 rounded-sm border-l-2 border-[#c5a059]/40 backdrop-blur-md">
        ...
        {wines.length === 0 && (
          <p className="text-center text-slate-600 font-sans font-bold tracking-[0.4em] py-48 opacity-50 uppercase text-xs">
            Cellar is currently empty
          </p>
        )}
      </div>
    </main>
