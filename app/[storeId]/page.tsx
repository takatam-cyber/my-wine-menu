...
export default function StoreMenu() {
  ...
  const filteredWines = useMemo(() => {
    return wines.filter((w: any) => {
      ...
      // 表示ON設定（空白や改行をトリムして大文字で比較）
      const isVisible = String(w.visible || 'ON').trim().toUpperCase() === 'ON';
      
      // 在庫あり（数値に変換して1以上、または在庫設定がない場合）
      const stockVal = parseInt(String(w.stock || '1').trim());
      const inStock = isNaN(stockVal) || stockVal > 0;
  ...
  const handleChat = async (directMsg?: string) => {
    ...
    try {
      ...
      setHistory(prev => [...prev, { role: 'assistant', content: "少々お待ちください。" }]);
    ...
    <main className="min-h-screen bg-[#fafafa] text-[#1a1a1a] font-sans pb-32">
          ...
          <div key={wine.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100">
              ...
              {wine.tags && (
                <div className="absolute top-4 left-4 flex gap-1 flex-wrap">
                  {wine.tags.split(',').map((tag: string) => (
                    <span key={tag} className="bg-white/20 backdrop-blur-md text-white text-[8px] font-black px-2 py-1 rounded-md uppercase">
                      #{tag.trim()}
                    </span>
                  ))}
                </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="space-y-3">
                <p className="text-sm text-slate-800 font-bold leading-relaxed italic">「{wine.menu_short}」</p>
                {wine.pairing && (
                  <div className="flex items-start gap-2 text-[#2f5d3a] bg-[#2f5d3a]/5 p-3 rounded-2xl">
                    ...
                    <p className="text-[11px] font-bold">相性: {wine.pairing}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="fixed bottom-6 left-0 right-0 flex justify-center z-50 pointer-events-none">
        <button 
          ...
          className="pointer-events-auto bg-[#2f5d3a] text-white px-8 py-5 rounded-full shadow-2xl flex items-center gap-3 animate-bounce"
        >
            ...
            {history.length === 0 ? (
              <div className="space-y-8">
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                  <p className="text-[#2f5d3a] font-black text-xl mb-3">いらっしゃいませ。</p>
                  <p className="text-slate-600 text-sm leading-relaxed font-medium">お料理やお好みに合う一本をご提案します。</p>
                </div>
              </div>
            ) : (
              history.map((h: any, i) => (
