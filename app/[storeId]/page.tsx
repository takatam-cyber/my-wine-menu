...
import { Sparkles, X, Send, Loader2, ChevronRight, Utensils, GlassWater, Tag } from 'lucide-react';
...
export default function StoreMenu() {
  ...
  // フィルタリングロジック: 色の選択 + 表示ON設定 + 在庫あり
  const filteredWines = useMemo(() => {
    return wines.filter((w: any) => {
      const matchColor = filterColor === 'ALL' || w.color === filterColor;
      const isVisible = w.visible === 'ON'; // 表示フラグのチェック
      const inStock = w.stock && parseInt(w.stock) > 0; // 在庫ありのチェック
    ...
    <main className="min-h-screen bg-[#fafafa] text-[#1a1a1a] font-sans pb-32">
      ...
      <div className="max-w-xl mx-auto px-6 space-y-10 mt-8">
        {filteredWines.map((wine: any) => (
          <div key={wine.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 transition-transform active:scale-[0.99]">
            {/* 画像エリア */}
            <div className="relative aspect-[16/10] bg-slate-50">
              ...
              {wine.tags && (
                <div className="absolute top-4 left-4 flex gap-1 flex-wrap">
                  {wine.tags.split(',').map((tag: string) => (
                    <span key={tag} className="bg-white/20 backdrop-blur-md text-white text-[8px] font-black px-2 py-1 rounded-md uppercase tracking-tighter">
                      #{tag.trim()}
                    </span>
                  ))}
                </div>
              ...
              <div className="absolute bottom-5 left-6 right-6 text-white">
                <p className="text-[10px] font-black uppercase opacity-80 mb-1">
                  {wine.country} {wine.vintage && `/ ${wine.vintage}`} {wine.alcohol && `/ ${wine.alcohol}%`}
                </p>
                <div className="flex justify-between items-end">
                  <h2 className="text-xl font-bold leading-tight">{wine.name_jp}</h2>
                  <div className="text-right">
                    <p className="text-[10px] opacity-60 font-bold">Bottle</p>
            ...
            <div className="p-6 space-y-5">
              <div className="flex flex-wrap gap-2 text-[9px] font-black uppercase text-slate-400">
                ...
                <span className="bg-slate-50 px-2 py-1 rounded border border-slate-100">{wine.grape}</span>
              </div>

              {/* 味わいドット表示 (33項目の各フィールドに対応) */}
              <div className="grid grid-cols-2 gap-x-8 gap-y-4 py-2 border-y border-slate-50">
                ...
                <FlavorDots 
                  label={wine.color === '赤' ? 'Tannin' : 'Aroma'} 
                  val={wine.color === '赤' ? wine.tannin : wine.aroma_intensity} 
                  color={wine.color === '赤' ? '#8b0000' : '#2f5d3a'} 
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                {wine.aroma_features && (
                  <div className="flex items-start gap-2 text-slate-500">
                    <Tag size={12} className="mt-1 flex-shrink-0" />
                    <p className="text-[11px] font-medium leading-tight">香り: {wine.aroma_features}</p>
                  </div>
                )}
                {wine.pairing && (
                  <div className="flex items-start gap-2 text-[#2f5d3a] bg-[#2f5d3a]/5 p-3 rounded-2xl">
                    <Utensils size={14} className="mt-0.5 flex-shrink-0" />
                    <p className="text-[11px] font-bold">おすすめの相性: {wine.pairing}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="fixed bottom-6 left-0 right-0 flex justify-center z-50 pointer-events-none">
        <button 
          ...
          className="pointer-events-auto bg-[#2f5d3a] text-white px-8 py-5 rounded-full shadow-2xl flex items-center gap-3 animate-bounce active:scale-95 transition-all"
        >
      ...
      {chatOpen && (
        <div className="fixed inset-0 bg-white z-[60] flex flex-col animate-in slide-in-from-bottom duration-300">
          <header className="flex justify-between items-center p-6 border-b bg-white">
            ...
            <button onClick={closeAndResetChat} className="text-slate-400 p-2 active:scale-90 transition-all">
              <X size={28}/>
            </button>
          </header>
            ...
            {history.length === 0 ? (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                  <p className="text-[#2f5d3a] font-black text-xl mb-3">Welcome.</p>
                  <p className="text-slate-600 text-sm leading-relaxed font-medium">
                    お客様のお好みやお料理に合わせ、リストの中から最高の1本をご提案いたします。
                  </p>
                </div>
              ...
              history.map((h: any, i) => (
                <div key={i} className={`flex ${h.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-5 rounded-2xl ${h.role === 'user' ? 'bg-[#2f5d3a] text-white shadow-lg' : 'bg-white text-slate-800 shadow-sm border border-slate-100'}`}>
                    <p className="text-sm font-bold leading-relaxed whitespace-pre-wrap">{h.content}</p>
                  </div>
                </div>
              ))
            )}
            {isTyping && <div className="flex justify-start"><div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm"><Loader2 className="animate-spin text-[#2f5d3a]" size={20} /></div></div>}
            <div ref={chatEndRef} />
          </div>
          ...
          <div className="p-6 bg-white border-t">
            <div className="relative max-w-xl mx-auto flex gap-2">
              <input 
                ...
                className="flex-1 bg-slate-100 rounded-2xl py-4 px-6 text-sm font-bold border-none outline-none focus:ring-2 ring-[#2f5d3a]/20" 
              />
              <button onClick={() => handleChat()} className="bg-[#2f5d3a] text-white p-4 rounded-2xl active:scale-90 transition-all shadow-lg">
                <Send size={20}/>
              </button>
            </div>
          </div>
...
function ChatOption({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex items-center gap-4 w-full p-6 bg-white border border-slate-100 rounded-2xl text-slate-700 font-bold text-sm active:scale-[0.98] shadow-sm hover:border-[#2f5d3a]/20 transition-all">
...
function FlavorDots({ label, val, color }: { label: string, val: string, color: string }) {
  const level = Math.min(5, Math.max(0, parseInt(val) || 3));
  return (
    <div className="space-y-1.5 flex-1">
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((step) => (
          <div 
            key={step} 
            className={`h-1 flex-1 rounded-full ${step <= level ? '' : 'bg-slate-100'}`} 
