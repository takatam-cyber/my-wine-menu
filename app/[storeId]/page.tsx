...
export default function StoreMenu() {
  ...
  const nextDiag = (key: string, val: string) => {
    ...
    setDiag(next);
    if (next.step === 5) {
      handleChat(`飲み方は「${next.format}」、予算は「${next.budget}」。種類は「${next.color}」で、特に「${next.style}」な味わいを求めています。今は「${next.scene}」の場面です。私を最高に満足させてくれる1本を教えてください。最後に必ず 【ID:番号】 を付けてください。`);
    }
  };
      ...
      <div className="space-y-4">
        ...
        {suggestedWines.map((wine: any) => (
          <button 
            ...
            className="w-full text-left bg-white rounded-2xl border-2 border-amber-100 shadow-md p-3 flex gap-4 animate-in zoom-in-95"
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-right text-[#2f5d3a] font-black text-[9px] mt-1">詳細へ移動 <ChevronRight size={10} className="inline"/></p>
            </div>
          </button>
        ))}
      </div>
  ...
  return (
    <main className="min-h-screen bg-[#fafafa] pb-32">
      {/* ヘッダー */}
      <header className="py-12 px-8 text-center bg-white border-b">
        <h1 className="text-2xl font-bold tracking-[0.2em] uppercase">{config.menu_name || 'WINE SELECTION'}</h1>
        <div className="h-[2px] w-8 bg-[#2f5d3a] mx-auto mt-2"></div>
      </header>

      {/* フィルタ */}
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-md px-6 py-4 border-b flex gap-2 overflow-x-auto no-scrollbar">
        {['ALL', '赤', '白', 'ロゼ', '泡'].map(c => (
          <button key={c} onClick={() => setFilterColor(c)} className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest ${filterColor === c ? 'bg-[#2f5d3a] text-white' : 'bg-slate-100 text-slate-400'}`}>{c}</button>
        ))}
      </div>
      ...
      <div className="max-w-xl mx-auto px-6 space-y-10 mt-8">
        {filteredWines.map((wine: any) => (
          <div key={wine.id} id={`wine-${wine.id}`} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 transition-all duration-500">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4 py-2 border-y border-slate-50 text-left">
              ...
              <p className="text-sm text-slate-800 font-bold italic leading-relaxed text-left">「{wine.menu_short}」</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 bg-white z-[60] flex flex-col animate-in slide-in-from-bottom duration-300">
          <header className="flex justify-between items-center p-6 border-b">
            ...
            <div className="flex items-center gap-4">
              {history.length > 0 && <button onClick={resetChat} className="text-[10px] font-black text-slate-400 flex items-center gap-1 uppercase transition-colors"><RotateCcw size={12}/>最初からやり直す</button>}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#f8f9f8]">
            {history.length === 0 ? (
              <div className="space-y-6">
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100"><p className="text-[#2f5d3a] font-black text-xl mb-2">{diag.step === 0 ? "今夜はどのようなスタイルで楽しまれますか？" : diag.step === 1 ? "ご希望の色はございますか？" : diag.step === 2 ? "味わいの方向性はいかがでしょう？" : diag.step === 3 ? "ご予算の目安を教えてください。" : "最後にお料理やシーンは？"}</p></div>
                <div className="grid grid-cols-1 gap-3">
                  ...
                  {diag.step === 2 && (
                    <>
                      {diag.color === '赤' ? (
                        <><ChatOption icon={<Heart className="text-red-500"/>} label="凝縮感のある果実味（リッチ系）" onClick={() => nextDiag('style', '果実味豊かなリッチ系')} /><ChatOption icon={<Sparkles className="text-purple-400"/>} label="洗練されたエレガンス（綺麗め系）" onClick={() => nextDiag('style', '洗練されたエレガント系')} /></>
                      ) : (
                        <><ChatOption icon={<GlassWater className="text-blue-400"/>} label="キリッと爽快なドライ（辛口系）" onClick={() => nextDiag('style', 'ドライな辛口系')} /><ChatOption icon={<Heart className="text-amber-500"/>} label="芳醇でフルーティー（華やか系）" onClick={() => nextDiag('style', '華やかなフルーティー系')} /></>
                      )}
                    </>
                  )}
                  {diag.step === 3 && <><ChatOption icon={<Banknote className="text-green-600"/>} label="カジュアルに (〜4,000円)" onClick={() => nextDiag('budget', 'カジュアル')} /><ChatOption icon={<Banknote className="text-amber-600"/>} label="標準的に (4,000〜10,000円)" onClick={() => nextDiag('budget', '標準')} /><ChatOption icon={<Banknote className="text-purple-600"/>} label="贅沢に (10,000円以上)" onClick={() => nextDiag('budget', 'プレミアム')} /></>}
              ...
              history.map((h: any, i) => (
                <div key={i} className={`flex ${h.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[90%] p-5 rounded-3xl ${h.role === 'user' ? 'bg-[#2f5d3a] text-white' : 'bg-white text-slate-800 border'}`}>
          ...
          <div className="p-6 bg-white border-t flex gap-2">
            <input type="text" value={message} onChange={e => setMessage(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleChat()} placeholder="他にご要望はありますか？" className="flex-1 bg-slate-100 rounded-2xl py-4 px-6 text-sm font-bold outline-none" />
...
function ChatOption({ icon, label, onClick }: { icon: any, label: string, onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex items-center gap-4 w-full p-6 bg-white border border-slate-100 rounded-2xl text-slate-700 font-bold text-sm shadow-sm active:scale-[0.98]">
