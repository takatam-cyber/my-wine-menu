export const runtime = 'edge'; // 【最優先】ランタイム指定を一番上に移動
...
export default function InventoryManager({ params }: { params: Promise<{ slug: string }> }) {
  // Next.js 15 の Promise 形式の params を展開
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-[3rem] p-12 shadow-2xl border border-slate-100 relative overflow-hidden">
          ...
          <div className="bg-slate-50 rounded-[2.5rem] p-10 border-4 border-dashed border-slate-200 space-y-6 text-center transition-colors hover:border-amber-200 group">
            ...
            <input 
              ...
              className="hidden" id="store-csv-upload" 
            />
            
            <label 
              htmlFor="store-csv-upload" 
              className="inline-block px-10 py-4 bg-white border-2 border-slate-900 rounded-full font-black text-xs cursor-pointer hover:bg-slate-900 hover:text-white transition-all shadow-md active:scale-95"
            >
              ファイルを選択
            </label>
            ...
            <button 
              ...
              className="w-full mt-6 py-6 bg-amber-500 text-black rounded-[1.5rem] font-black text-xl shadow-2xl shadow-amber-500/30 hover:bg-amber-400 hover:-translate-y-1 transition-all disabled:opacity-50 disabled:translate-y-0 flex items-center justify-center gap-3 active:scale-95"
            >
          ...
          {status === 'success' && (
            <div className="mt-8 p-6 bg-emerald-50 text-emerald-700 rounded-2xl flex items-center gap-4 font-black border border-emerald-100 animate-in fade-in slide-in-from-top-4">
              <CheckCircle2 size={24} className="animate-bounce" /> 
              <span>メニューの更新に成功しました！</span>
            </div>
          )}
          {status === 'error' && (
            <div className="mt-8 p-6 bg-red-50 text-red-700 rounded-2xl flex items-center gap-4 font-black border border-red-100">
              <span>エラーが発生しました。CSV形式を確認してください。</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-end px-4">
            <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.3em]">Current Lineup</h2>
            <span className="text-[10px] font-black text-slate-300 uppercase">{currentWines.length} Items</span>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-4 mt-2">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-slate-300 uppercase">Bottle</span>
                      <span className="font-bold text-slate-700 text-sm">¥{Number(wine.price_bottle).toLocaleString()}</span>
                    </div>
                    <div className="w-[1px] h-8 bg-slate-100 self-end" />
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-slate-300 uppercase">Glass</span>
                      <span className="font-bold text-slate-700 text-sm">¥{Number(wine.price_glass).toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              </div>
