      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-30 px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button 
            ...
            className="px-6 py-3.5 bg-amber-500 text-black rounded-2xl font-black flex items-center gap-2 hover:bg-amber-400 transition-all shadow-xl shadow-amber-500/20 text-sm whitespace-nowrap active:scale-95"
          >
            <Plus size={18}/> 新規店舗登録
          </button>
        </header>
        ...
        <div className="p-8 max-w-7xl mx-auto space-y-10">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 pb-12">
                ...
                <div key={store.slug} className="group bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden hover:shadow-2xl transition-all duration-300">
                  <div className="p-8 space-y-6">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 font-mono text-xs mt-1">URL: /{store.slug}</p>
                    </div>
                    <button 
                      onClick={() => router.push(`/admin/inventory/${store.slug}`)} 
                      className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-sm hover:bg-black transition-all flex items-center justify-center gap-3 shadow-xl active:scale-95"
                    >
