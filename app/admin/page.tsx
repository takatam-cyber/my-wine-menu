...
import { 
  Camera, Loader2, Wine as WineIcon, Edit3, LogOut, Save, Trash2, Settings, Globe, ChevronRight
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const handleScan = async (e: any) => {
    ...
    } catch (err: any) { alert("解析失敗。もう一度試してください。"); } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-black p-6">
      <div className="bg-white p-10 rounded-[2.5rem] w-full max-w-sm">
        ...
        <form onSubmit={(e:any) => { e.preventDefault(); setIsLoggedIn(true); localStorage.setItem('wine_store_email', auth.email); }} className="space-y-4">
          ...
          <button className="w-full bg-black text-white py-5 rounded-2xl font-black text-lg active:scale-95 transition-all">ログイン</button>
        </form>
      </div>
    </div>
  );
    ...
    <div className="max-w-3xl mx-auto p-6 bg-slate-50 min-h-screen text-black pb-24 font-sans">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl mb-8 border border-slate-100 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input type="text" value={config.menu_name} onChange={e => setConfig({...config, menu_name: e.target.value})} className="w-full p-4 bg-slate-50 rounded-xl border-2 font-bold" placeholder="メニュー名 (店舗名)" />
          <div className="flex items-center bg-slate-50 rounded-xl border-2 px-4 focus-within:border-amber-500">
            ...
            <input type="text" value={config.slug} onChange={e => setConfig({...config, slug: e.target.value})} className="flex-1 p-3 bg-transparent font-bold outline-none" placeholder="URLスラッグ" />
          </div>
        </div>
        <button onClick={async () => { await fetch('/api/config', { method: 'POST', headers: { 'x-store-id': auth.email }, body: JSON.stringify(config) }); alert("保存しました"); }} className="w-full bg-amber-500 text-white py-4 rounded-2xl font-black shadow-lg">設定を保存</button>
      </div>
      ...
      <div className="mt-16 p-10 bg-slate-900 text-white rounded-[3.5rem] shadow-2xl relative overflow-hidden group">
        ...
        <a href={`/${config.slug || auth.email}`} target="_blank" className="inline-flex items-center gap-3 bg-[#c5a059] text-black px-10 py-5 rounded-2xl text-[10px] font-black tracking-[0.2em] uppercase shadow-xl transition-all active:scale-95">
