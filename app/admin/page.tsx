...
export default function AdminPage() {
  ...
  const handleScan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 3. AIの返答をクリーンアップ（\_ を _ に戻し、JSONを抽出）
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-5">
            <div>
              ...
              <input type="text" value={newWine.name_jp} onChange={e => setNewWine({...newWine, name_jp: e.target.value})} className="w-full p-4 bg-slate-100 rounded-xl border-2 border-transparent focus:border-slate-400 outline-none text-black font-bold text-lg transition shadow-inner" />
            </div>
            <div>
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest">Wine Name (Alphabet)</label>
              <input type="text" value={newWine.name_en} onChange={e => setNewWine({...newWine, name_en: e.target.value})} className="w-full p-4 bg-slate-100 rounded-xl border-2 border-transparent focus:border-slate-400 outline-none text-black font-bold text-lg transition shadow-inner" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">国</label>
                <input type="text" value={newWine.country} onChange={e => setNewWine({...newWine, country: e.target.value})} className="w-full p-4 bg-slate-100 rounded-xl border-2 border-transparent focus:border-slate-400 outline-none text-black font-bold transition shadow-inner" />
              </div>
              <div>
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">産地</label>
                <input type="text" value={newWine.region} onChange={e => setNewWine({...newWine, region: e.target.value})} className="w-full p-4 bg-slate-100 rounded-xl border-2 border-transparent focus:border-slate-400 outline-none text-black font-bold transition shadow-inner" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">品種</label>
                <input type="text" value={newWine.grape} onChange={e => setNewWine({...newWine, grape: e.target.value})} className="w-full p-4 bg-slate-100 rounded-xl border-2 border-transparent focus:border-slate-400 outline-none text-black font-bold transition shadow-inner" />
              </div>
              <div>
                <label className="text-xs font-black text-slate-400 uppercase tracking-widest">ヴィンテージ</label>
                <input type="text" value={newWine.vintage} onChange={e => setNewWine({...newWine, vintage: e.target.value})} className="w-full p-4 bg-slate-100 rounded-xl border-2 border-transparent focus:border-slate-400 outline-none text-black font-bold transition shadow-inner" />
              </div>
            </div>
          </div>
          ...
          <div className="col-span-full space-y-5 border-t border-slate-100 pt-6">
            <div>
              ...
              <textarea value={newWine.taste} onChange={e => setNewWine({...newWine, taste: e.target.value})} className="w-full p-4 bg-slate-100 rounded-xl border-2 border-transparent focus:border-slate-400 outline-none text-black font-bold h-24 transition shadow-inner" />
            </div>
            <div>
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest">ワイン紹介コメント (日本語)</label>
              <textarea value={newWine.description} onChange={e => setNewWine({...newWine, description: e.target.value})} className="w-full p-4 bg-slate-100 rounded-xl border-2 border-transparent focus:border-slate-400 outline-none text-black font-bold h-32 transition shadow-inner" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {wines.map((wine: any) => (
            <div key={wine.id} className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm relative group">
              <button onClick={() => handleDelete(wine.id)} className="absolute top-2 right-2 p-2 bg-red-50 text-red-500 rounded-full opacity-0 group-hover:opacity-100 transition z-10">
                <Trash2 size={16} />
              </button>
              ...
              <p className="text-sm font-black truncate text-black">{wine.name_jp}</p>
