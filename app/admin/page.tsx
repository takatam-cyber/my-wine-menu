...
export default function AdminPage() {
  ...
  // データ取得の整合性を保つため、常にフルメールアドレスをIDとして使用
  useEffect(() => {
  ...
  const handleScan = async (e: any) => {
    ...
    try {
      ...
      if (scanData.result && scanData.result !== "{}") {
        ...
        alert("画像は取り込みましたが、文字情報の解析に失敗しました。残りは手動で補完してください。");
      }
    } catch (err: any) { alert("通信エラーが発生しましたが、画像は保持されました。"); } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0d0e12] p-6 text-black">
      <div className="bg-white p-10 rounded-[2.5rem] w-full max-w-sm shadow-2xl">
        ...
        <form onSubmit={(e:any) => { e.preventDefault(); setIsLoggedIn(true); localStorage.setItem('wine_store_email', auth.email); }} className="space-y-4">
          ...
          <input type="password" placeholder="パスワード" value={auth.pass} onChange={e => setAuth({...auth, pass: e.target.value})} className="w-full p-4 bg-slate-100 rounded-2xl font-bold border-none outline-none focus:ring-2 ring-amber-500" required />
      ...
      <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-slate-100 mb-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-3">
            ...
            <input type="text" placeholder="名前 (日本語)" value={newWine.name_jp} onChange={e => setNewWine({...newWine, name_jp: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border-2 outline-none focus:border-amber-500" />
            <input type="text" placeholder="Name (English)" value={newWine.name_en} onChange={e => setNewWine({...newWine, name_en: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border-2 outline-none focus:border-amber-500" />
