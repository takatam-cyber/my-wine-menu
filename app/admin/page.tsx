...
import { 
  Camera, Loader2, Wine as WineIcon, Edit3, LogOut, Save, Mail, Lock, Trash2, Settings, ChevronRight, Download, Upload, ExternalLink
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const currentStoreId = auth.email.split('@')[0] || auth.email;
  ...
  const exportCSV = () => {
    const headers = "id,name_jp,name_en,country,region,grape,color,type,vintage,price,cost,stock,advice,aroma,pairing,sweetness,body,acidity,tannin,image\n";
  ...
  const handleScan = async (e: any) => {
    ...
    try {
      ...
      if (scanData.result && scanData.result !== "{}") {
        ...
        alert("画像は取り込みましたが、文字解析に失敗しました。手動で入力してください。");
      }
    } catch (err: any) { alert("解析中にエラーが発生しました。画像のみ取り込まれた可能性があります。"); } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0d0e12] p-6 text-black">
      <div className="bg-white p-10 rounded-[2.5rem] w-full max-w-sm shadow-2xl">
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          <input type="email" placeholder="メールアドレス" value={auth.email} onChange={e => setAuth({...auth, email: e.target.value})} className="w-full p-4 bg-slate-100 rounded-2xl font-bold border-none outline-none focus:ring-2 ring-amber-500" required />
          {mode === 'login' && <input type="password" placeholder="パスワード" value={auth.pass} onChange={e => setAuth({...auth, pass: e.target.value})} className="w-full p-4 bg-slate-100 rounded-2xl font-bold border-none outline-none focus:ring-2 ring-amber-500" required />}
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-slate-50 min-h-screen text-black pb-24 font-sans">
      ...
      <div className="bg-white p-8 rounded-[3rem] shadow-2xl border border-slate-100 mb-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-3">
            <select value={newWine.color} onChange={e => setNewWine({...newWine, color: e.target.value})} className="w-full p-4 bg-slate-100 rounded-2xl font-black border-none outline-none">
              <option value="赤">Red</option><option value="白">White</option><option value="ロゼ">Rosé</option><option value="泡">Sparkling</option>
            </select>
            <input type="text" placeholder="名前 (日)" value={newWine.name_jp} onChange={e => setNewWine({...newWine, name_jp: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border-2 outline-none focus:border-amber-500" />
            <input type="text" placeholder="English Name" value={newWine.name_en} onChange={e => setNewWine({...newWine, name_en: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl border-2 outline-none focus:border-amber-500" />
      ...
      <div className="mt-16 p-10 bg-slate-900 text-white rounded-[3.5rem] shadow-2xl relative overflow-hidden group">
        ...
        <a href={`/${currentStoreId}`} target="_blank" className="inline-flex items-center gap-3 bg-[#c5a059] text-black px-10 py-5 rounded-2xl text-[10px] font-black tracking-[0.2em] uppercase active:scale-95 transition-all shadow-xl">
